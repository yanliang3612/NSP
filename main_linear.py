...
def main():
    ...
    for fold in range(args.repetitions):
        ...
        output_size_num = 1
        Linear_model = Linear(input_size=input_size_num, output_size=output_size_num).to(device)
        print('Linear model:', Linear_model)
        print('model.parameters:', Linear_model.parameters)
        ...
        for epoch in range(args.epochs):
            output = Linear_model(run_train_x).to(device)
            ...
            if loss < prev_loss:
                torch.save(Linear_model.state_dict(), 'Linear_model.pt')  # save model parameters to files
                prev_loss = loss
        ...
        linear_model = Linear_model.eval()  # switch to testing model

        # prediction on test dataset

        testsize = test_y.shape[0]
        output_test = linear_model(test_x).to(device)
