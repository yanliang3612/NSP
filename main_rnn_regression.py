...
from src.utils import set_random_seeds,summary_mae,summary_mse
...
def main():
    ...
    for fold in range(args.repetitions):
        ...
        output_size_num = 1
        RNN_model = RNN(input_size=input_size_num, hidden_size=args.hidden, output_size=output_size_num, num_layers=args.layers).to(device)
        print('RNN model:', RNN_model)
        print('model.parameters:', RNN_model.parameters)
        ...
        for epoch in range(args.epochs):
            output = RNN_model(run_train_x,None).to(device)
            ...
            if loss < prev_loss:
                torch.save(RNN_model.state_dict(), 'RNN_model.pt')  # save model parameters to files
                prev_loss = loss
        ...
        rnn_model = RNN_model.eval()  # switch to testing model

        # prediction on test dataset

        testsize = test_y.shape[0]
        output_test = rnn_model(test_x,None).to(device)
